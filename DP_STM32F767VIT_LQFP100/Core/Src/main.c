...
static void USBH_UserProcess(USBH_HandleTypeDef * phost, uint8_t id)
{
  ...
  case HOST_USER_DISCONNECTION:
    ...
    if (FATFS_UnLinkDriver(USBpath) != 0)
    {
        //Fail_ctr++;
    }
    //USB_State = 1;
...
//    Appli_state = APPLICATION_READY;
  ...
  case HOST_USER_CONNECTION:
    ...
    {
      if (f_mount(&USBFatFs, (TCHAR const*)USBpath, 0) != FR_OK)          // (TCHAR const*)USBDISKPath
      {
          //Fail_ctr++;
      }
    }
...
*/
 ...
 // HAL_Delay(10000);
  ...
  //ringbuff_init(&txLogFileRingBuffer,txLogFile,LOGGING_FILE_RING_BUFFER_SIZE);
  ...
  if(FATFS_LinkDriver(&USBH_Driver, USBpath) == 0){
	  ...
	  resUSB = f_mount(&USBFatFs, (TCHAR const*)USBpath, 0);

	  if(resUSB != FR_OK){
	  		  send_uart3("ERROR - USB - MOUNT\n");
	  }
	  else{

		  resUSB = f_open(&USBFile, "test.txt", FA_CREATE_ALWAYS | FA_WRITE);

		  if(resUSB != FR_OK){
			  send_uart3("ERROR - USB - TEST FILE CREATE\n");
		  }
  ...
  if(FATFS_LinkDriver(&SD_Driver, SDpath) == 0){
	  ...
	  resSD = f_mount(&SDFatFs, (TCHAR const*)SDpath, 1);

	  if(resSD != FR_OK){
		  send_uart3("ERROR - SD - MOUNT\n");
	  }
	  else{
		  ...
		  {
			  // FatFs Format Error
			  send_uart3("ERROR - SD - FORMAT\n");
		  }

		  else{
			  resSD = f_open(&SDFile, "test.txt", FA_CREATE_ALWAYS | FA_WRITE);

			  if(resSD != FR_OK){
				  send_uart3("ERROR - SD - TEST FILE CREATE\n");
			  }
		  }
  ...
  while (1)
  {
	  // cycle measurement
	  		...
	  		//if ( ( __HAL_TIM_GET_COUNTER(&htim5) - previousMeasuringTime ) >=  settings.measuringInterval) {
	  			...
	  			/*
	  			previousValues[currentValuePosition % SAMPLES] = 0.05;
	  			previousValuesRange[currentValuePosition % SAMPLES]= 1;
	  			currentValuePosition++;
				*/

	  			// Internal ADC convertor - 12b
	  			//adc_sample_internal();
	  			...
	  			/*
	  			if(isMeasuring == 0){
	  				if (currentRange == 0)		// nA
	  					measuredValueCurrent = (measuredValue / 1000000);
	  				else if (currentRange == 1)		// uA
	  					measuredValueCurrent = (measuredValue / 1000);
	  				else if (currentRange == 2)		// mA
	  					measuredValueCurrent = (measuredValue);
	  				else
	  					// leaving as is indicating error during operation
	  					measuredValueCurrent = measuredValue;

	  				//sprintf(uartBufferTx, "Measurement:  %d %d %6.10f %6.16f\n", measuringNumber, currentRange, measuredValue, measuredValueCurrent);

	  				//sprintf(uartBufferTx, "Measurement:  %d %6.10f %6.16f\n", currentRange, measuredValue, measuredValueCurrent);
	  		...
	  		//snprintf(uartBufferTx, 25, "\n\nMeasured value: %.8f\n\n", measuredValue);
	  		//sprintf(uartBufferTx, "\n\nMeasured value: %6.10f\n\n", measuredValue);
